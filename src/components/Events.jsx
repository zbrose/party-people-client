        ...
        <Card style={{ width: "18rem" }}>
          ...
          <Card.Body>
            ...
            <ListGroup variant="flush">
              ...
              <ListGroup.Item>
                {dayjs
                  .tz(event.date, "America/New_York")
                  .format("MMMM D, YYYY")}
              </ListGroup.Item>
              <ListGroup.Item>
            ...
            <Button
              href={currentUser ? `/events/${event._id}` : "/login"}
              className="btn-block text-uppercase mb-2 rounded-pill shadow-sm"
              variant="outline-dark"
            >
              {currentUser ? "See Details" : "Log In to see details"}
            </Button>
          </Card.Body>
        </Card>
      </div>
  ...
  return (
    <>
      <Tabs
        ...
        activeKey={key}
        onSelect={(f) => filterEvents(f)}
      >
        <Tab eventKey="All" title="All">
          <div className="flex-box tab-style">
            {!eventsList[0] ? (
              <h3 className="white">No Events Listed In This Category</h3>
            ) : (
              eventsList
            )}
          </div>
        ...
        <Tab eventKey="Study" title="Study">
          <div className="flex-box study-tab tab-style">
            {!eventsList[0] ? (
              <h3 className="white">No Events Listed In This Category</h3>
            ) : (
              eventsList
